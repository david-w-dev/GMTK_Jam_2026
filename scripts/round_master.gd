extends Node3D
class_name RoundMaster

enum RoundOutcome { WON, LOST }
signal round_ended(outcome: RoundOutcome)

@export var wall_rise_speed: float = 1.0
@export var wall_scene: PackedScene


class Params:
	## The number of ticks in the round
	var num_ticks: int
	## The length of each tick in seconds
	var tick_duration_s: float
	## The number of enemies in this round
	var num_enemies: int
	## The round number, to show how many rounds have been played so far (1-indexed)
	var round_number: int
	## The player
	var player: PlayerController
	## The level
	var level: Node

var in_progress = true
var params: Params = null
var ticks_remaining: int
var enemies_remaining: int
var wall_y: float = -WallGenerator.WALL_MODEL_HEIGHT


func setup(p: Params) -> void:
	params = p
	ticks_remaining = params.num_ticks
	enemies_remaining = params.num_enemies
	params.player.connect("death", _on_player_death)


func end_round(outcome: RoundOutcome) -> void:
	if not in_progress:
		# This could happen if multiple enemies hit the player at once, for example
		return
	in_progress = false
	$tick_timer.stop()
	round_ended.emit(outcome)


func _ready() -> void:
	assert(params != null, "RoundMaster added with null params")
	# Create walls
	for pos in WallGenerator.get_random_walls():
		var wall = wall_scene.instantiate()
		add_child(wall)
		WallGenerator.set_wall_position(wall, pos, wall_y)

	set_mock_hud()

func spawn_enemies():
	for i in range(params.num_enemies):
		add_child($spawner.spawn_enemy())
	$tick_timer.start(params.tick_duration_s)

func set_mock_hud() -> void:
	# Note: this HUD is just for debug and testing
	$mock_hud.text = "round %d\nticks remaining: %d\nenemies remaining: %d" \
						% [params.round_number, ticks_remaining, enemies_remaining]


func _process(delta: float) -> void:
	if wall_y < 0:
		wall_y = min(0, wall_y + wall_rise_speed * delta)
		# TODO: if this gets more complicated, use a start chart. For now that would
		# be overkill
		if wall_y == 0:
			spawn_enemies()

	for child in get_children():
		if child is Wall:
			child.position.y = wall_y
		if child is Enemy:
			child.run_towards(params.player.global_position)


func _on_tick_timer_timeout() -> void:
	params.level.switch_panel(params.num_ticks - ticks_remaining, "off") # 0-indexed
	ticks_remaining -= 1
	set_mock_hud()
	if ticks_remaining == 0:
		end_round(RoundOutcome.LOST)


func _on_player_death() -> void:
	end_round(RoundOutcome.LOST)


func _on_spawner_enemy_killed() -> void:
	enemies_remaining -= 1
	set_mock_hud()
	if enemies_remaining == 0:
		end_round(RoundOutcome.WON)
