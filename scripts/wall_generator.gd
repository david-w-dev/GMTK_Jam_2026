...
const GRID_SIZE_CELLS: int = int(floor(2.0 * PanelUtils.RADIUS / WALL_TARGET_WIDTH))
...
# placement script.
const SIDE_LENGTH: float = 2 * PanelUtils.RADIUS
...
static func get_random_walls() -> Array[WallPosition]:
	...
	var center_cell = Vector2(float(GRID_SIZE_CELLS) / 2, float(GRID_SIZE_CELLS) / 2)
